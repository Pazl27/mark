...
mod tests {
    ...
    fn test_background_searcher_creation() {
        ...
        let searcher = BackgroundSearcher::new(dir_path, vec![], false, false);
    ...
    fn test_background_searcher_finds_markdown_files() {
        ...
        let mut searcher =
            BackgroundSearcher::new(dir_path.to_str().unwrap(), vec![], false, false).unwrap();
    ...
    fn test_background_searcher_hidden_files_behavior() {
        ...
        let file_names: Vec<String> = found_files_no_hidden
            .iter()
            .map(|f| f.name.clone())
            .collect();
        assert!(file_names.iter().any(|name| name.ends_with("root.md")));
        assert!(file_names
            .iter()
            .any(|name| name.ends_with("docs/public.md")));
        ...
        let file_names_with_hidden: Vec<String> = found_files_with_hidden
            .iter()
            .map(|f| f.name.clone())
            .collect();
        assert!(file_names_with_hidden
            .iter()
            .any(|name| name.ends_with("root.md")));
        assert!(file_names_with_hidden
            .iter()
            .any(|name| name.ends_with("docs/public.md")));
        assert!(file_names_with_hidden
            .iter()
            .any(|name| name.ends_with(".hidden/secret.md")));
    }
    ...
    fn test_background_searcher_streaming_behavior() {
        ...
        for _ in 0..200 {
            // Give more time for streaming
            let messages = searcher.try_recv();
            if !messages.is_empty() && !completed {
                // If we get some files but not Finished yet, we're streaming
                let has_files = messages
                    .iter()
                    .any(|m| matches!(m, SearchMessage::FileFound(_)));
    ...
    fn test_background_searcher_nonexistent_directory() {
        let searcher = BackgroundSearcher::new("/nonexistent/directory/path", vec![], false, false);
