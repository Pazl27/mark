...
pub use parser::{AstNode, Parser, parse_tokens, parse_markdown as parser_parse_markdown, parse_markdown_or_default as parser_parse_markdown_or_default};
        ...
        if let AstNode::Document { children } = ast {
            ...
            if let AstNode::Heading { level, content } = &children[0] {
                ...
                let text_content = content.iter()
                    .filter_map(|node| if let AstNode::Text(text) = node { Some(text.as_str()) } else { None })
                    .collect::<Vec<_>>()
                    .join("");
...
###### H6"#;
        ...
        if let AstNode::Document { children } = ast {
            let headings: Vec<_> = children.iter()
                .filter_map(|child| if let AstNode::Heading { level, .. } = child { Some(*level) } else { None })
                .collect();
    ...
    fn test_strikethrough() {
        ...
        if let AstNode::Document { children } = ast {
            if let AstNode::Paragraph { content } = &children[0] {
                let has_strikethrough = content.iter().any(|node| matches!(node, AstNode::Strikethrough(_)));
            ...
            if let AstNode::Paragraph { content } = &children[0] {
                ...
                let has_inline_code = children.iter().any(|child| {
                    if let AstNode::Paragraph { content } = child {
                        content.iter().any(|node| matches!(node, AstNode::InlineCode(_)))
                    } else { false }
                });
...
3. Third item"#;
        ...
        if let AstNode::Document { children } = ast {
            let has_ordered_list = children.iter().any(|child| {
                matches!(child, AstNode::List { ordered: true, items } if items.len() == 3)
            });
...
- Third item"#;
        ...
        if let AstNode::Document { children } = ast {
            ...
            assert!(has_unordered_list, "Should contain unordered list with 3 items");
        }
    }
...
> Second line of quote"#;
        ...
        if let AstNode::Document { children } = ast {
            let has_blockquote = children.iter().any(|child| matches!(child, AstNode::BlockQuote { .. }));
        ...
        if let AstNode::Document { children } = ast {
            let has_code_block = children.iter().any(|child| {
                if let AstNode::CodeBlock { language, code } = child {
                    let lang_matches = language.as_ref().map(|l| l.contains("rust")).unwrap_or(false);
...
```"#;
        ...
        if let AstNode::Document { children } = ast {
            let has_code_block = children.iter().any(|child| {
                matches!(child, AstNode::CodeBlock { language: None, .. })
            });
    ...
    fn test_horizontal_rule() {
        ...
        if let AstNode::Document { children } = ast {
            let has_hr = children.iter().any(|child| matches!(child, AstNode::HorizontalRule));
    ...
    fn test_links() {
        ...
        if let AstNode::Document { children } = ast {
            if let AstNode::Paragraph { content } = &children[0] {
                let links: Vec<_> = content.iter()
                    .filter_map(|node| if let AstNode::Link { url, .. } = node { Some(url) } else { None })
                    .collect();
    ...
    fn test_images() {
        ...
        if let AstNode::Document { children } = ast {
            if let AstNode::Paragraph { content } = &children[0] {
                let images: Vec<_> = content.iter()
                    .filter_map(|node| if let AstNode::Image { url, .. } = node { Some(url) } else { None })
                    .collect();
...
![Image](test.jpg)"#;
        ...
        if let AstNode::Document { children } = ast {
            ...
            let has_heading = children.iter().any(|child| matches!(child, AstNode::Heading { .. }));
            let has_paragraph = children.iter().any(|child| matches!(child, AstNode::Paragraph { .. }));
            let has_unordered_list = children.iter().any(|child| matches!(child, AstNode::List { ordered: false, .. }));
            let has_ordered_list = children.iter().any(|child| matches!(child, AstNode::List { ordered: true, .. }));
            let has_blockquote = children.iter().any(|child| matches!(child, AstNode::BlockQuote { .. }));
            let has_code_block = children.iter().any(|child| matches!(child, AstNode::CodeBlock { .. }));
            ...
            if let AstNode::Paragraph { content } = &children[0] {
                let has_nested = content.iter().any(|node| {
                    if let AstNode::Bold(bold_content) = node {
                        bold_content.iter().any(|inner| matches!(inner, AstNode::Italic(_)))
                    } else { false }
                });
    ...
    fn test_whitespace_only_document() {
        ...
        if let AstNode::Document { children } = ast {
            ...
            assert!(children.len() <= 1, "Should have at most one child for whitespace-only document");
        }
    }
    ...
    fn test_multiple_newlines() {
        ...
        if let AstNode::Document { children } = ast {
            let paragraphs: Vec<_> = children.iter()
                .filter(|child| matches!(child, AstNode::Paragraph { .. }))
                .collect();
            assert!(paragraphs.len() >= 1, "Should contain at least one paragraph");
...
| Cell 1   | Cell 2   |"#;
        ...
        if let AstNode::Document { children } = ast {
            let has_table = children.iter().any(|child| matches!(child, AstNode::Table { .. }));
    ...
    fn test_complex_nesting() {
        ...
        if let AstNode::Document { children } = ast {
            if let AstNode::BlockQuote { content } = &children[0] {
                let has_bold = content.iter().any(|node| matches!(node, AstNode::Bold(_)));
                let has_code = content.iter().any(|node| matches!(node, AstNode::InlineCode(_)));
                assert!(has_bold || has_code, "Should contain nested elements in blockquote");
            }
        }
    }
        ...
        if let AstNode::Document { children } = ast {
            if let AstNode::Heading { level, content } = &children[0] {
                ...
                let text_parts: Vec<String> = content.iter()
                    .filter_map(|node| if let AstNode::Text(text) = node { Some(text.clone()) } else { None })
                    .collect();
                ...
                for level in 2..=6 {
                    ...
                    if let AstNode::Document { children } = test_ast {
                        if let AstNode::Heading { level: parsed_level, .. } = &children[0] {
...
```"#;
        ...
        if let AstNode::Document { children } = ast {
            let code_block = children.iter().find(|child| matches!(child, AstNode::CodeBlock { .. }));
...
```"#;
    ...
    fn test_inline_code_content_validation() {
        ...
        if let AstNode::Document { children } = ast {
            if let AstNode::Paragraph { content } = &children[0] {
                let inline_codes: Vec<String> = content.iter()
                    .filter_map(|node| if let AstNode::InlineCode(code) = node { Some(code.clone()) } else { None })
                    .collect();
                
                assert_eq!(inline_codes.len(), 2, "Should have exactly 2 inline code blocks");
...
- Unordered item three"#;
            ...
            if let AstNode::List { ordered, items } = ordered_list.unwrap() {
                ...
                if let AstNode::ListItem { content } = &items[0] {
                    ...
                    let text_content = content.iter()
                        ...
                        .collect::<Vec<_>>()
                        .join("");
                    assert!(text_content.contains("First item"), "Should contain 'First item'");
                }
    ...
    fn test_emphasis_content_validation() {
        ...
        if let AstNode::Document { children } = ast {
            if let AstNode::Paragraph { content } = &children[0] {
                // Check bold content
                let bold_nodes: Vec<&Vec<AstNode>> = content.iter()
                    .filter_map(|node| if let AstNode::Bold(content) = node { Some(content) } else { None })
                    .collect();
                ...
                let italic_nodes: Vec<&Vec<AstNode>> = content.iter()
                    .filter_map(|node| if let AstNode::Italic(content) = node { Some(content) } else { None })
                    .collect();
                assert_eq!(italic_nodes.len(), 1, "Should have exactly one italic element");
                ...
                let strike_nodes: Vec<&Vec<AstNode>> = content.iter()
                    .filter_map(|node| if let AstNode::Strikethrough(content) = node { Some(content) } else { None })
                    .collect();
                assert_eq!(strike_nodes.len(), 1, "Should have exactly one strikethrough element");
    ...
    fn test_link_content_validation() {
        ...
        if let AstNode::Document { children } = ast {
            if let AstNode::Paragraph { content } = &children[0] {
                let links: Vec<(&Vec<AstNode>, &String)> = content.iter()
                    .filter_map(|node| if let AstNode::Link { text, url } = node { Some((text, url)) } else { None })
                    .collect();
    ...
    fn test_image_content_validation() {
        ...
        if let AstNode::Document { children } = ast {
            if let AstNode::Paragraph { content } = &children[0] {
                let images: Vec<(&Vec<AstNode>, &String)> = content.iter()
                    .filter_map(|node| if let AstNode::Image { alt, url } = node { Some((alt, url)) } else { None })
                    .collect();
                ...
                assert_eq!(*first_url, "https://www.rustlang.org/logos/rustlogox512.png");
...
> > This is a nested quote."#;
        ...
        if let AstNode::Document { children } = ast {
            let blockquote = children.iter().find(|child| matches!(child, AstNode::BlockQuote { .. }));
            ...
            if let AstNode::BlockQuote { content } = blockquote.unwrap() {
                ...
                // Check text content
                let text_nodes: Vec<&str> = content.iter()
                    .filter_map(|node| if let AstNode::Text(text) = node { Some(text.as_str()) } else { None })
                    .collect();
                
                let full_text = text_nodes.join("");
                assert!(full_text.contains("blockquote"), "Should contain 'blockquote'");
                assert!(full_text.contains("multiple lines"), "Should contain 'multiple lines'");
            }
        }
    }
...
| Carol | 35 | JavaScript |"#;
        ...
        if let AstNode::Document { children } = ast {
            let table = children.iter().find(|child| matches!(child, AstNode::Table { .. }));
            ...
            if let AstNode::Table { headers, rows } = table.unwrap() {
                ...
                let header_texts: Vec<String> = headers.iter().map(|header| {
                ...
                if let Some(first_row) = rows.get(0) {
                    ...
                    let cell_texts: Vec<String> = first_row.iter().map(|cell| {
...
> - Another item with *emphasis*"#;
        ...
        if let AstNode::Document { children } = ast {
            ...
            let heading = children.iter().find(|child| matches!(child, AstNode::Heading { .. }));
            ...
            let paragraph = children.iter().find(|child| matches!(child, AstNode::Paragraph { .. }));
            if let Some(AstNode::Paragraph { content }) = paragraph {
                ...
                // Validate link URL
                if let Some(AstNode::Link { url, .. }) = content.iter().find(|node| matches!(node, AstNode::Link { .. })) {
...
> This quote contains **bold** and *italic* text."#;
        ...
        if let AstNode::Document { children } = ast {
            ...
            let headings: Vec<_> = children.iter().filter(|c| matches!(c, AstNode::Heading { .. })).collect();
            let paragraphs: Vec<_> = children.iter().filter(|c| matches!(c, AstNode::Paragraph { .. })).collect();
            let code_blocks: Vec<_> = children.iter().filter(|c| matches!(c, AstNode::CodeBlock { .. })).collect();
            let lists: Vec<_> = children.iter().filter(|c| matches!(c, AstNode::List { .. })).collect();
            let blockquotes: Vec<_> = children.iter().filter(|c| matches!(c, AstNode::BlockQuote { .. })).collect();
            ...
            assert_eq!(lists.len(), 2, "Should have two lists (ordered and unordered)");
            ...
            let heading_levels: Vec<u8> = headings.iter()
                .filter_map(|h| if let AstNode::Heading { level, .. } = h { Some(*level) } else { None })
                .collect();
            ...
            let ordered_list = lists.iter().find(|l| {
                if let AstNode::List { ordered, .. } = l { *ordered } else { false }
            });
