...
impl AstNode {
    ...
    pub fn text_content(&self) -> String {
        match self {
            ...
            | AstNode::TableCell { content: children } => children
                .iter()
                .map(|child| child.text_content())
                .collect::<Vec<_>>()
                .join(""),
            AstNode::Link { text, .. } | AstNode::Image { alt: text, .. } => text
                .iter()
                .map(|child| child.text_content())
                .collect::<Vec<_>>()
                .join(""),
        ...
        match self {
            ...
            | AstNode::TableCell { content: children } => {
                children.len()
                    + children
                        .iter()
                        .map(|child| child.count_children())
                        .sum::<usize>()
            }
            AstNode::Link { text, .. } | AstNode::Image { alt: text, .. } => {
                text.len()
                    + text
                        .iter()
                        .map(|child| child.count_children())
            ...
            AstNode::List { items, .. } => {
                items.len()
                    + items
                        .iter()
                        .map(|item| item.count_children())
                        .sum::<usize>()
            }
            AstNode::Table { headers, rows } => {
                let header_count =
                    headers.len() + headers.iter().map(|h| h.count_children()).sum::<usize>();
                let row_count = rows
                    .iter()
                    .map(|row| {
